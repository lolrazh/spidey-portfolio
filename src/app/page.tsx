...
const GALLERY_DATA = galleryConfig as GalleryConfig;

const GALLERY_IMAGES = (GALLERY_DATA.images ?? []).map((image) => {
...
const HERO_IMAGE_ENTRY = GALLERY_IMAGES.find((image) => getFilename(image.file) === getFilename(GALLERY_DATA.heroImage));
const HERO_IMAGE_BASENAME = HERO_IMAGE_ENTRY?.basename ?? getBasename(GALLERY_DATA.heroImage);
const HERO_IMAGE_ALT = HERO_IMAGE_ENTRY?.alt?.trim() || 'Spidey hero portrait';

const POLAROID_IMAGE_ENTRY = GALLERY_IMAGES.find((image) => getFilename(image.file) === getFilename(GALLERY_DATA.polaroidImage));
const POLAROID_IMAGE_BASENAME = POLAROID_IMAGE_ENTRY?.basename ?? getBasename(GALLERY_DATA.polaroidImage);
const POLAROID_IMAGE_ALT = POLAROID_IMAGE_ENTRY?.alt?.trim() || 'Spidey polaroid portrait';

const PORTFOLIO_IMAGES = GALLERY_IMAGES.filter((image) => {
  const include = image.includeInGallery !== false;
  const isHero = image.basename === HERO_IMAGE_BASENAME;
  const isPolaroid = image.basename === POLAROID_IMAGE_BASENAME;
  return include && !isHero && !isPolaroid;
});
...
export default function SpideyPortfolio() {
  ...
  const lightboxImagePaths = PORTFOLIO_IMAGES.map(image =>
              ...
              >
                <picture>
                  ...
                  <img
                    ...
                    alt={image.alt?.trim() || `Portfolio image ${i + 1}`}
                    loading="lazy"
                    decoding="async"
