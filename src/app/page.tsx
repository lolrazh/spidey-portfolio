...
type GalleryImageWithBasename = GalleryImage & { basename: string };

type GalleryConfig = {
  hero?: GalleryImage | null;
  polaroids?: GalleryImage[];
  gallery?: GalleryImage[];
};

const extractBasename = (imagePath: string) => {
  const fileName = imagePath.split(/[\\/]/).pop() ?? "";
  return fileName.replace(/\.[^/.]+$/, "");
};

const galleryConfig = galleryData as GalleryConfig;

const heroImage = galleryConfig.hero ?? null;
const heroBasename = heroImage ? extractBasename(heroImage.image) : null;

const polaroidImages: GalleryImageWithBasename[] = (galleryConfig.polaroids ?? [])
  .map((item) => {
    const basename = extractBasename(item.image);
    if (!basename) return null;
    return { ...item, basename };
  })
  .filter((item): item is GalleryImageWithBasename => item !== null);

const portfolioImages: GalleryImageWithBasename[] = (galleryConfig.gallery ?? [])
  .map((item) => {
    const basename = extractBasename(item.image);
    if (!basename) return null;
    return { ...item, basename };
...
export default function SpideyPortfolio() {
  ...
  // Prepare image paths for Lightboxes
  const lightboxImagePaths = portfolioImages.map(image =>
    `/optimized-images/${image.basename}-1200w.webp`
  );
  const lightboxImageAlts = portfolioImages.map((image, index) =>
    image.alt ?? `Portfolio image ${index + 1}`
  );
  const polaroidLightboxPaths = polaroidImages.map(image =>
    `/optimized-images/${image.basename}-1200w.webp`
  );
  const polaroidLightboxAlts = polaroidImages.map((image, index) =>
    image.alt ?? `Polaroid ${index + 1}`
  );
  const hasPolaroids = polaroidLightboxPaths.length > 0;
  ...
  return (
    <main className="min-h-screen flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-start">
            ...
            <div className="md:col-span-6 flex flex-col">
              ...
              <div className="flex items-center space-x-8 mt-4 mb-8">
                {/* Polaroids Button */}
                <button
                  className="font-sans text-sm uppercase tracking-wider border border-current px-3 py-1 hover:opacity-70 transition-opacity duration-200 disabled:opacity-40 disabled:cursor-not-allowed"
                  disabled={!hasPolaroids}
            ...
            <div className="md:col-span-5 relative w-full cursor-default">
              {heroBasename ? (
                <picture>
                  <source
                    type="image/avif"
                    srcSet={generateSrcSet(heroBasename, 'avif')}
                    sizes={imageSizes} // Reuse sizes prop
                  />
                  <source
                    type="image/webp"
                    srcSet={generateSrcSet(heroBasename, 'webp')}
                    sizes={imageSizes}
                  />
                  {/* Fallback img (can use Next Image or regular img) */}
                  <img
                    src={`/optimized-images/${heroBasename}-1200w.webp`} // Fallback src
                    alt={heroImage?.alt ?? "Portfolio hero image"}
                ...
                <div className="aspect-[3/4] w-full bg-neutral-100" aria-hidden="true" />
              )}
            </div>
          </div>
        </div>
      ...
      <section className="w-full px-1">
        <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-2"> 
          {portfolioImages.map((image, i) => {
            const layoutId = `card-image-${image.basename}`;
            return (
              <motion.div
                key={`portfolio-${i}-${image.basename}`}
                layoutId={layoutId}
              ...
              >
                <picture>
                  <source
                    type="image/avif"
                    srcSet={generateSrcSet(image.basename, 'avif')}
                    sizes={imageSizes}
                  />
                  <source
                    type="image/webp"
                    srcSet={generateSrcSet(image.basename, 'webp')}
                    sizes={imageSizes}
                  />
                  {/* Fallback img */}
                  <img
                    src={`/optimized-images/${image.basename}-828w.webp`}
      ...
      {/* Polaroid Lightbox */}
      <AnimatePresence>
        {polaroidLightboxOpen && hasPolaroids && (
          <Lightbox
            key="polaroid-lightbox"
            isOpen={polaroidLightboxOpen}
            onClose={closePolaroidLightbox}
            images={polaroidLightboxPaths}
            imageAlts={polaroidLightboxAlts}
            currentIndex={0}
          />
        )}
      </AnimatePresence>
